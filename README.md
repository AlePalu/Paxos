...
Every node processing a `NAMINGREQUEST` before forwarding it to the name server, issues a ticket to track the ongoing request. If a response to such request is not received before a given timeout, the name server is considered death, and a bully election is started. The node which for first discovered the naming failure sends to all other nodes having an UUID greater than its an `ELECT` message, issuing a new ticket to keep track of the ongoing election. On reception of an `ELECT` message, if the UUID of the recipient is greater than the one of the sender, it sends back to the sender a `BULLYSUPPRESS` message, by which the sender of the `ELECT` removes any ticket issued as a consequence of the `ELECT` transmission.
The only node which will have a ticket expired, will be the one having the greatest UUID on the network, since it's the only node which hasn't received a `BULLYSUPPRESS`. Such node has just been elected to be the new name server. It informs all other nodes about this, by broadcasting a `COORD` message. In turn, when a node receives such `COORD` message, it updates its name server reference by making it to point to the node hosting the new name server.
On the elected node, a new instance of `NamingRequestHandler` is launched, ready to serve naming requests as the old one. The list of active nodes is restored from the last `NAMINGREPLY` received.
